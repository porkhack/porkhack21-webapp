...
export const state = {
  ...
  list: ({ selected, operations }, state) => {
    ...
    return _.omitBy(
      _.mapValues(operations, ({ name }, id) => {
  ...
  operations: (local, state) => {
    ...
    if (_.get(state, `OADAManager.connected`) === true) {
      ...
      operations = _.omitBy(
        _.get(
          state,
          `oada.${currentConnection}.bookmarks.seasons.2020.operations`
        ) || {},
        (o, k) => {
          return _.startsWith(k, "_");
        }
      ); //TODO year
    } else {
  ...
  selectedOperationId: ({ selected, operations }, state) => {
    ...
    if (selectedId === null && _.keys(operations).length > 0)
      selectedId = _.keys(operations)[0];
    return selectedId;
  },
